...
Mailin.prototype.start = function (options, callback) {
    ...
    if(options && options.smtpOptions && options.smtpOptions.disabledCommands) {
        configuration.smtpOptions.disabledCommands = options.smtpOptions.disabledCommands
    }
    ...
    function parseEmail(connection) {
        ...
        return MailParser(fs.createReadStream(connection.mailPath)).then(function (mail) {
